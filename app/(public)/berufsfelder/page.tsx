...
import { ArrowLeft, Briefcase, HeartPulse, Hammer, Search, ShoppingBag } from "lucide-react";
    ...
    <div className="space-y-10">
      <PhoneFrame className="max-w-[330px]">
        <div className="flex items-center justify-between">
          ...
          <p className="text-[1.95rem] text-slate-900 [font-family:Georgia,serif]">Berufsfelder</p>
          ...
          {featured.map((item) => {
            ...
            return (
              <Link
                key={item.title}
                href={item.href}
                className="group relative block overflow-hidden rounded-[1.35rem] border border-[#eadfce] bg-[#fbf4ea] shadow-sm"
              >
                ...
                <div className="relative z-20 flex min-h-[5.8rem] items-center gap-3 p-3">
                ...
                <Image
                  ...
                  width={1400}
                  height={700}
                  className="absolute inset-y-0 right-0 h-full w-[58%] object-cover object-center transition-transform duration-300 group-hover:scale-105"
                />
              </Link>
            );
          })}
