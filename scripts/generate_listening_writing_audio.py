#!/usr/bin/env python3
"""Generiert echte .m4a-Hörtexte aus lib/listening-writing-data.ts mit macOS say/afconvert."""
...
DATA_FILE = ROOT / "lib" / "listening-writing-data.ts"
...
def parse_tasks(text: str) -> dict[str, str]:
    ...
    tasks: dict[str, str] = {}
    id_pattern = re.compile(r'id:\s*"(hlw-\d+)"', re.MULTILINE)
...
def main() -> None:
    ...
    print(f"{len(tasks)} Hören-und-Schreiben-Hörtexte gefunden. Generiere Audio in {OUTPUT_DIR} …")
